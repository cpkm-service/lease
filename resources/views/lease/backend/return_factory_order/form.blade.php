...
@section('options')
    ...
    <div class="row mb-2">
        <div class="col-12">
            ...
            @if($detail->status?->id == 1 && (!$detail->overhaul_order))
                <a href="{{route('backend.lease.overhaul_order.create', ['sourceable_type' => \Cpkm\Lease\Models\LeaseReturnFactoryOrder::class, 'sourceable_id' => $detail->id])}}" class="btn btn-primary">轉檢修單</a>
                <a href="#" class="btn btn-success">其租他收入單</a>
            @else
                @if($detail->overhaul_order)
                <a href="{{route('backend.lease.overhaul_order.show', ['overhaul_order' => $detail->overhaul_order->id])}}" class="btn btn-info" target="_blank">檢修單</a>
                @endif
            @endif
        </div>
    </div>
...
<script>
    var url = "{{route('backend.lease.return_factory_order.index')}}";
